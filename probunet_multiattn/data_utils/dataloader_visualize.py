...
class BratsDataset(Dataset):
    ...
    def __prepare_data(self, index=False):
        self.data = glob.glob(self.src_dir + "/*seg.nii.gz") # BraTS20_Training_091_t1ce.nii.gz
        self.data = [d[:-11] for d in self.data]
    ...
    def __getitem__(self, ix):
        ...
        t1_gz = nib.load('{}_t1.nii.gz'.format(pid_path))
        t2_gz = nib.load('{}_t2.nii.gz'.format(pid_path))
        t1ce_gz = nib.load('{}_t1ce.nii.gz'.format(pid_path))
        flair_gz = nib.load('{}_flair.nii.gz'.format(pid_path))
        seg_gz = nib.load('{}_seg.nii.gz'.format(pid_path))
        ...
        seg = seg_gz.get_fdata()
        
        wt = np.expand_dims(seg > 0, axis=0)
        tc = np.expand_dims(np.logical_or(seg == 1, seg == 4), axis=0)
        et = np.expand_dims(seg == 4, axis=0)
        
        out = np.concatenate([wt, tc, et], axis=0).astype(np.float32)
        out = resize(np.transpose(out, axes=(3, 0, 1, 2)))
        ...
        output_volume = torch.from_numpy(out.copy())
        
        # Return a tuple of two elements: a tuple of inputs and the output tensor.
        return (input_volume, output_volume, pid)
